...
public class A_Variable {
	...
	public static void main(String[] args) {
		...
		// 실행항 메서드가 있는 클레스 생성 (new)
		A_Variable variable = new A_Variable ();
		
		// 생성한 클레스로 메서드 실행 (호출)
		// variable.printValue();
		//variable.variableTast();
		variable.constant();
	}
	...
	public void variableTast() {
		/*
		 * 변수의 선언
		 * 
		 * 자료형 변수명;
		 * - 자료형 : 어떤 값을 저장할지 , 어떤 크기의 메모리를 할당받을지에 대한 정보
		 * - 변수명 :변수의 이름으로, 값을 읽고, 쓰고 지우는 작업을 할떄 사용
		 * 
		 * 변수의 초가화
		 * 
		 * 변수명 = 값
		 * - 변수에 처음으로 값을 저장하는 것
		 * - '=' 오른쪽 값을 왼쪽 변수에 대입 (저장)하는 연산자
		 * 
		 * */
		// 지본 자료형(Primitive type) : 실제 값을 저장 
		// 스택 메모리에 생성된 공간에 실제 변수값이 저장
		// 1. 논리형 : 1byte = 8bit
		boolean isTrue;// 변수 선언 
		isTrue = true; // 변수 초기화
		
		boolean isFalse = false; // 변수 선언과 함께 초가화!
...
//		bNum = 128; -> 에러 발생 ! 오버 플로우(overflow)
		short sNum = 128; // 2byte(16bit) -> 2^16
		int iNum = 922337283; // 4byte(32bit) (기본)
		long lNum = 9223125464L; // 8byte(64bit) - 숫자 뒤에 L(l)을 입력
		...
		// * 참조 자료형 (클래스) : 4byte, 기본형을 제외한 나머지, 주소 값 저장
...
//		int public;
	...
	public void constant() {
		 ...
		 * fainl 자료형 뱐수명;
		 * 
		 * - 변하지 않는 값으로 특정 뱐수의 값이 변경되는 걸 원하지 않을 떄 사용
