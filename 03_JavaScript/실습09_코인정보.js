...
const dataFetch = async () => {
  ...
  /*
  data.forEach((item) => {
    //startsWith : 시작하는 단어, includes : 포함여부
  ...
  */
  ...
  krwMarkets.forEach((item) => {
    tickerData.find((ticker) => ticker.market === item.market);
    market.innerHTML += `<h2>
        ${item.korean_name}(${item.market})</h2>
      <p>현재가 : ${ticker.trade_price.toLocaleString()}원</p>
      <p>24시간 거래량 :${ticker.acc_trade_volume_24h.toFoixed(2)}
      <p>전일 대비 :${ticker.change === 'RISE' ? "상승" : ticker.change === 'FALL' ? "하락" : "보합"} (${(ticker.change_rate * 100.toFixed(2))}%)</p>
      `;
  });
};
dataFetch();
