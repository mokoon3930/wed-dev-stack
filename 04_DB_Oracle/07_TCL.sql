/*
    ...
    -트렌잭션을 제어하는 언어
    -데이터 베이스는 데이터 변경 사항(INSERT, UPDATE, DELETE)
    ...
    예시) ATM기
        ...
        5. 현금 인출
        6. 완료
        - 각각의 업무들이 묶어서 하나의 작업 단위로 만드는것을 트랜잭션이라고 한다.

    COMMIT : 모든 작업들을 정상적으로 처리하겠다고 확정
    ROLLBACK : 모든 작없들을 취소하겠다.(마지막 COMMIT)
    SAVEPOINT : 임시 저장 시점을 정하고 ROLLBACK 진행시 SAVEPOINT 까지 일부 롤백
*/
