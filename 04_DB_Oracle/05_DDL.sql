...
CREATE TABLE MEM_GRADE(
    GRADE_CODE GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    GENDE_NAME VARCHAR2(30) NOT NULL
);
INSERT INTO MEM_GRADE (GENDE_NAME) VALUES ('일반회원');
INSERT INTO MEM_GRADE (GENDE_NAME) VALUES ('우수호원');
INSERT INTO MEM_GRADE (GENDE_NAME) VALUES ('특별회원');


CREATE TABLE MEM_GRADER(
    MEM_NO NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    MEM_ID VARCHAR2(20) NOT NULL UNIQUE,
    MEM_PWD VARCHAR2(20) NOT NULL,
    MEM_NAME VARCHAR2(20) NOT NULL,
...
INSERT INTO MEM_GRADE(MEM_ID, MEM_PWD, MEM_NAME, GENDER, PHONE, EMAIL, MEM_DATE,  GRADE_ID) VALUES('user01', 'pass01', '윤의진', '남', '010-1111-2222', 'aaa@naver.com', '25/06/05', 1);
INSERT INTO MEM_GRADE(MEM_ID, MEM_PWD, MEM_NAME, MEM_DATE, GENDER,  GRADE_ID)
            VALUES('user02', 'pass02', '이진용', SYSDATE, '남', 2);
INSERT INTO MEM_PRI2(MEM_ID, MEM_PWD, MEM_NAME, GENDER, MEM_DATE, EMAIL,  GRADE_ID)
            VALUES('user03', 'pass03', '곽병현', '남', SYSDATE, 'bbb@google.com', 10);
SELECT * FROM MEM_PRI2;

/*
    FOREING KEY(외래키) 제약조건
    - 오래키 역활을 하는 커럼의 부여하는 제약 조건
    - 다른 테이블에 존제하는 값만 들어와야 되는 특정 컬럼에 부여하는 제약조건

    --> 다른 테이블을 참조한다고 표현
    --> 주로 FOREING KEY 제약조건에 의해 테이블 간의 관계가 형성
*/
...
INSERT INTO MEM_MEMBER2(MEM_ID, MEM_PWD, MEM_NAME, GENDER, PHONE, EMAIL, MEM_DATE,  GRADE_ID) VALUES('user01', 'pass01', '윤의진', '남', '010-1111-2222', 'aaa@naver.com', '25/06/05', 1);
INSERT INTO MEM_MEMBER2(MEM_ID, MEM_PWD, MEM_NAME, MEM_DATE, GENDER,  GRADE_ID)
            VALUES('user02', 'pass02', '이진용', SYSDATE, '남', 2);
INSERT INTO MEM_MEMBER2(MEM_ID, MEM_PWD, MEM_NAME, GENDER, MEM_DATE, EMAIL,  GRADE_ID)
            VALUES('user03', 'pass03', '곽병현', '남', SYSDATE, 'bbb@google.com', 10);



-- 데이터 삭제는 : DELETE FROM 테이블명 WHERE 조건;
-- MEM_GRADE 테이블에서 GRADE_CODE가 1인 걸 삭제
DELETE FROM MEM_GRADE WHERE GRADE_CODE = 1; 

--부모 테이블(MEM_GRADE)에서 데이터를 삭제 할경우 문제 발생!
--이유가 자식 레코드가 있어서 작제 되지 않음!
...
/*
    자식 테이블 생성시 왜래키 제약 조건 부여할때 삭제 옵션 지정
    * 삭제옵션 : 부모테이블에 데이터 삭제시 스 데이터를
    사용하고 있는 자식 테이블 어떻게 할지 처리

    -ON DELETE RESTRICTED(기본값)
    : 자식데이터로 쓰이는 부모데이터는 삭제 아예 안되게끔
    -ON DELETE SET NULL
    :부모 데이터 삭제시 해당 데이터를 쓰고있는 자식 데이터의 값을  NULL로 변경
    -ON DELETE CASCADE
    : 부모 데이터 삭제시 해당 데이터를 쓰고 있는 자식 데이터도 같이 삭제 시킴
*/
...
/*
    DEFAUULT 기본값
    -제약조건 아님!
    -컬럼을 선정하지 않고 INSERT  시 NULL 이 아닌 기본값을
    INSERT 하고자 할 떄 세팅해둘수 있는값
*/
CREATE TABLE MEMBER(
    MEM_NO NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    MEM_NAME VARCHAR2(20) NOT NULL,
    MEME_AEG NUMBER DEFAULT 10,
...
CREATE TABLE RENT(
    ...
    BK_NO NUMBER REFERENCES BOOK(BK_NO) NO DELETE SET NULL
);
INSERT INTO RENT (MEMBER, BK_NO)
VALUES(1, 1);
INSERT INTO RENT (MEMBER, BK_NO)
VALUES(2, 3);
INSERT INTO RENT (MEMBER, BK_NO)
