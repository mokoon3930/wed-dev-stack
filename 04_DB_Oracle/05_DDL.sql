...
/*
    ...
    - 테이블에서 각 행들을 식별하기 위해 
         사용될 컬럼에 부여하는 제약조건 (식별자 역할)
      EX) 회원번호, 학번, 사원번호, 부서코드, 직급코드, 주문번호
            예약번호, 운송장번호 ....
    - PRIMARY KEY 제약조건을 부여하면 그 컬럼은 
        자동으로 NOT NULL + UNIQUE 제약조건이 설정됨
...
DELETE FROM MEM_GRADE WHERE GRADE_CODE = 3;

-- 부모테이블(MEM_GRADE)에서 데이터 값을 삭제할 경우 문제 발생!
-- 이유가 자식 레코드가 있어서 삭제되지 않음!
DELETE FROM MEM_MEMBER2 WHERE MEM_NO = 8;
...
CREATE TABLE MEM_MEMBER2(
    ...
    FOREIGN KEY (GRADE_ID) 
        REFERENCES MEM_GRADE(GRADE_CODE) ON DELETE CASCADE
);
INSERT INTO MEM_MEMBER2(MEM_ID, MEM_PWD, MEM_NAME, GRADE_ID)
VALUES('user01', 'pass01', '윤의진', 1);
INSERT INTO MEM_MEMBER2(MEM_ID, MEM_PWD, MEM_NAME, GRADE_ID)
...
/*
    ...
    -데이터 값이 있어도 같이 삭제됨 (삭제도니 컬럼은 복구 불가능)
*/
ALTER TABLE USER_INFO DROP COLUMN LNAME;

-- 부모키 (PRIMARY KEY) 참도ㅈ되고 있는 커럼이 있다면 삭제 불가능!
ALTER TABLE BOOK DROP COLUMN BK_NO;
-- 참조되고 있는 컬럼이 삭제되 있을시 가능;
ALTER TABLE PUBLISHER DROP COLUMN BK_NO;

/*
    1-4. 컬럼명 변경
    RENAME COLUMN 기존 데이터명 TO 변경할 컬럼명;
*/
...
/*
    ...
    FOREIGN KEY : FORRIGN KEY (컬럼명) REFRTENCES 테이블명 (컬럼명);
    UNIQUE : UNIQUE (컬럼명);
...
WHERE TABLE_NAME='DEPT_DOPY'; 

/*
    2-3. 제약 조건명 변경
    RENAME CONSTRAINT 지존 제약 조건명 TO 변경할 제약 조건명;
*/
...
/*
    3. 테이블명 변경
    ALTER TABLE 기존 테이블명 TO 변경할 테이블명;
