/*
    ...
    -SQL 문장내에서 변수의 정의, 조건처리(), 반복처리() 등을 지원하는 SQL 의 단점을 보안
    -다수의 SQL 문을 한번에 실행하는 기능(BLOCK 구조)


    *블록(BLOCK) : 명령어를 모아둔 PL/SQL 프로그래의 기본 단위

    *PL/SQL 구조
    - [선언부(DECLARE SECTION)] : DECLARE 시작, 변수와 상수를 선언 및 초기화
    - 실행부(EXECUTABLE SECTION) : BEGIN 시작, SQL문 또는 제어문(조건문, 반복문)등의 로직을 기술하는부분
    - [예외처리부(EXCEPTION SECTION)] : EXCEPTION 시작, 예외 시 해결하기 위한 구문을 미리 기술
*/
SET SERVEROUTPUT ON;

BEGIN 
    DBMS_OUTPUT.PUT_LINE('HELLO OEACLE');
END;
/
-- / <---무조건 명시!
...
/*
    DECLARE 선언부
    - 변수 및 상수를 선언 하는 공간(선언과 동시에 초기화도 가능)
...
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
    DTITLE DEPARTMANT.DEPT_TITLE%TYPE;
BEGIN
...
*/
-- 사번을 입력 받은후 해당 사원의 이름, 보너스 출력
-- 보너스를 받지 않는  사원은 보너스 출력 전애 ' 보너스를 지급 받지 않는 사원입니다' 출력
DECLARE
...
/*
    IF ~ ELSE ~ ELSE
    IF 조건식 1 THEN 실행내용1
    ELSIF 조건식2 THEN 실행내용 2
    ...
    ELSE 실행내용
    END IF;
*/
-- 사용자에게 점수를 입력받아 SCORE 변수 (데이터 타입 : NUMBER)에 저장후
-- 학점은 입력된 점수에 따라 GRADE 변수 (데이터타입 : CHAR (1))에 저장
-- 출력은 '당신의 점수는  95점이고, 학점은 A입니다.'
...
BEGIN
    ...
    IF EMP.SALARY >= 50000000 THEN GRADE := '고급';
    ELSIF EMP.SALARY >= 30000000 THEN GRADE := '중급';
    ELSE GRADE := '초급';
    END IF;
...
/*
    ...
    END CASE;
*/
-- 사번을 입력받은 후 수서 코드에 따라 
-- D1인 경우는 개발팀, D2인경우는 디자인팀, D3인 경우는 기획팀, 그외 서비스팀
-- 출력 : 해당 사원이름 'OOO는 개발팀 입니다'
DECLARE
EMP EMPLOYEE%ROWTYPE;
DNAME VARCHAR2(30);
BEGIN
...
/*
    ...
    EXCEPTION
        ...
        WHEN 예외명2  THEN 예외처리구문 2
        ...

    * 올라클에서 미리 저의되어 있는 시스템 예외
    - NO_DATA_FOUND : SELECT 한 결과가 한 행도 없는경우
    - TOO_MANY_ROWS : SELECT 한 결과가 한 행이 리턴 되어야 하는데 여러 행인 경우
    - ZERO_DIVIDE : 숫자를 1으로 나눌 때
    - DUP_VAL_ON_INDEX : UNIQUE 데약조건에 위배되었을때
    ...
*/
BEGIN
...
-- EMPLOYEE 에서 부서코드(DEPT_CODE) 를 입력 받아서 사원을 조회 하려고 할때
-- NO_DATA_FOUND : 조회 결과가 없습니다
-- TOO_MANY_ROWS : 너무 많은 행이 조회 되었습니다
...
EXCEPTION
    ...
    WHEN TOO_MANY_ROWS THEN DBMS_OUTPUT.PUT_LINE('너무 많은 행이 조회 되었습니다');
END;
/
