/*
    ...
    * 캐시메모리
    ...
    - 매번 호출될 때마다 새로 번호를 생성하는게 아니라 캐시메모리 공간에 미리 생성된 값들을 가져다 쓸수 있음 (속도가 빨라짐)
    - 접속이 해재되념 -> 캐시매모리에 미리 만들어 둔 번호들은 다날라감
...
-- 현재 계정이 소유하고 있는 시퀀스들의 구조를 보자고 할 때
...
*/
-- CURRVAL는 한번이라도 NEXTVAL가 수행하지 않으면 가져올수 없음!
...
/*
    ...
    * VIEW 옵션들
    - OR REPLACE : 뷰 생성시 기존의 중복된 이름의 뷰가 없다면 새로 뷰 생성
                    기존의 중복된 뷰가 있다면 해당 뷰 변경하는 옵션
    - FORCE : 서브 쿼리에 기술된 테이블이 존제하지 않는 테이블이어도 뷰 생성
    - NOFORCE : 서브 쿼리에 기술된 테이블이 존제하여야만 뷰 생성 (기본값)
    - WITH CHECK OPTION : 서브 쿼리에 기술된 조건에 부합하지 않는 값으로 수정하는 경우 오류 발생
    - WITH READ ONLY : 뷰에 대해 조회만 가능 (DML 수행 불가)
*/
-- 관리자 계정 CREATE VIEW 권한부여
...
/*
    ...
    - 서브쿼리의 SELECT 절에 함수식이나 산술연산식이 기술되어있었을 경우 반드시 별칭 부여!
*/
-- 사원의 사번, 사원명, 직급명, 성별, 근무년수 조회
-- 뷰명 : VM_EMP_JOP
-- 1. 일부만 별칭 부여할 시 서브쿼리에서

CREATE OR REPLACE VIEW  VM_EMP_JOP
AS SELECT 
...
-- 2. 뷰 생성시 모든 컬럼에 별칭 부여      
 CREATE OR REPLACE VIEW  VM_EMP_JOP ("사번", "사원명", "직급명", "성별", "근무년수")
    AS SELECT 
            EMP_ID, EMP_NAME, JOB_NAME,
...
-- 조인뷰에 의하여 기본 테이블 의외의 컬럼을 수정하지 못한다
...
-- WITH CHECK OPTION : 서브 쿼리에 기술된 조건에 부합하지 않는 값으로 수정시 오류 발생
CREATE OR REPLACE VIEW VW_EMP
AS SELECT * FROM EMPLOYEE
    WHERE SALARY >= 3000000
WITH CHECK OPTION;

-- WITH CHECK OPTION이 있는경우 해당 조건에 위가되어 변경 불가
...
-- WITH READ ONLY : 뷰에 대한 조회만 가능 (DML 수행불가)
...
-- 데이터가 얼마 없으면 오라클에서 굳이 인덱스 조회 사용해서 데이터 조회 X
SELECT * 
FROM USER_INFO
WHERE USER_ID = 1;
