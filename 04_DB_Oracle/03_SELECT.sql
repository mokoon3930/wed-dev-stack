...
SELECT MBTI, AGE, AVG(AGE), SUM(AGE), COUNT(*)
FROM USER_INFO
GROUP BY MBTI, AGE;
...
/*
    ...
    1 FROM     테이블명
    2 WHERE    조건식
    3 GROUP BY 그룹 기준에 해당하는 컬럼 | 합수
    4 HAVING   조건식 (그룹 함수)
    6 ORDER BY 컬럼 | 별칭 | 컬럼순번 (숫자)

*/
--EMPLOYEE에서 부서별 평균 급여가 300만원 이상인 직원의
--평균 급여(SALARY)를 조회(부서 코드 - DEPT_CODE)
--DEPT_CODE가 NULL이 아닌 경우
SELECT DEPT_CODE,
        TO_CHAR(FLOOR(AVG(NVL(SALARY, 0))), '9,999,999') "300이상 평균급여"
FROM EMPLOYEE
WHERE DEPT_CODE IS NOT NULL
GROUP BY DEPT_CODE
HAVING AVG(NVL(SALARY, 0)) >= 3000000;
--직급별(JOB_CODE) 총 그여의 합이 1000만원 이상인 직급만 조회
